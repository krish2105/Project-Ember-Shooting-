...
## Driveable vehicle integration — 0.3.6-dev.1

- Converted all four licensed sports cars from set dressing into enterable Chaos Modular Vehicles. The nearest car is placed at insertion; proximity interaction accepts `E`, `F` or the controller top button and rejects distant or player-occupied cars.
- Added controller-owned throttle, brake-to-reverse, steering and handbrake input that remains active after possession. Added a safe capsule-tested exit, re-possession of the original character and an entry debounce so the entering button press cannot immediately eject the player.
- Added an Ember third-person vehicle camera with collision, lag and bounded mouse/right-stick orbit. This replaces the example camera that resolved below the World Partition road during the first automated drive capture.
- Added a vehicle-specific HUD, speed display, driving control guide, spatial procedural engine loop and a seated mannequin pose clone while preserving the on-foot character for exit/checkpoint recovery.
- Verification complete: Mac Editor arm64 compile passed; `Saved/Automation/20260714T223219Z` contains 10 passed / 0 failed, including the vehicle simulation/access contract; content validation completed with 0 errors and one existing device-profile priority warning. The final archive at `Releases/Development/20260715T030000Z` cooked and packaged successfully. A fresh extraction is thin arm64, deep/strict ad-hoc-signature valid, and its native Metal `EmberVehicleSmoke` route entered the car, accepted throttle/steering, moved 270.7 cm at 4.0 KPH, captured the corrected rear chase view and exited successfully. ZIP SHA-256: `dfcb13242fa86daca6cd364b06a6e2064dc119633cb8afaf7a042895177ee996`.

## Gameplay input focus hotfix — 0.3.7-dev.1
...
- Final verification: the arm64 game target compiled and linked; `Saved/Automation/20260715T122435Z` contains 10 passed / 0 failed / 0 warnings. Clean package `Releases/Development/20260715T123000Z` passed ZIP integrity, thin-arm64, Info.plist and deep/strict ad-hoc-signature checks after a fresh extraction. The exact extracted archive's native Metal `EmberVehicleSmoke` route recorded `yawDelta=-0.00` while idle, accepted right/left/center steering phases, moved 11.16 m at 7.9 KPH, exited, and returned the input bridge to on-foot mode. ZIP SHA-256: `d19ba455fe1949b1fe4b7f94eb510fcddea57d2f2650ba7d4c8acdb40ea3f501`.
- GitHub release `v0.3.9-dev.1` was downloaded again through its public asset URL; the 415,045,820-byte download matched the validated local archive byte-for-byte and reproduced the same SHA-256.
