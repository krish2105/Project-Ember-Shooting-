...
- Verification before packaging: Mac Editor arm64 compile passed; `Saved/Automation/20260714T223219Z` contains 10 passed / 0 failed, including the vehicle simulation/access contract; content validation completed with 0 errors and one existing device-profile priority warning. The native Metal `EmberVehicleSmoke` route entered the car, applied throttle/steering, moved 229.1 cm, settled at road height, captured the corrected chase view and exited successfully. Packaged/fresh-download evidence is recorded in the release notes after archive verification.
